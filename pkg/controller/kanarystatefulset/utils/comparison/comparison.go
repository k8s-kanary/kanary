...
func CompareDeploymentMD5Hash(hash string, dep *apps.Deployment) bool {
	if val, ok := dep.Annotations[string(kanaryv1alpha1.MD5KanaryStatefulsetAnnotationKey)]; ok && val == hash {
		return true
	}
	return false
}
...
func SetMD5DeploymentSpecAnnotation(kd *kanaryv1alpha1.KanaryStatefulset, dep *apps.Deployment) (string, error) {
	...
	dep.Annotations[string(kanaryv1alpha1.MD5KanaryStatefulsetAnnotationKey)] = md5Spec
	return md5Spec, nil
}
