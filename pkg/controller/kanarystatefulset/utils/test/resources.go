...
func NewDeployment(name, namespace string, replicas int32, options *NewDeploymentOptions) *appsv1beta1.Deployment {
	...
	newDep := &appsv1beta1.Deployment{
		...
		ObjectMeta: metav1.ObjectMeta{
			Name:        name,
			Namespace:   namespace,
			Annotations: map[string]string{string(kanaryv1alpha1.MD5KanaryStatefulsetAnnotationKey): md5},
		},
		Spec: *spec,
	}
...
func NewPod(name, namespace, hash string, options *NewPodOptions) *corev1.Pod {
	newPod := &corev1.Pod{
		...
		ObjectMeta: metav1.ObjectMeta{
			Name:        name,
			Namespace:   namespace,
			Annotations: map[string]string{string(kanaryv1alpha1.MD5KanaryStatefulsetAnnotationKey): hash},
		},
	}
