...
func GetNextValidationCheckDuration(kd *v1alpha1.KanaryStatefulset) time.Duration {
...
func GetValidationDeadLine(kd *v1alpha1.KanaryStatefulset) time.Time {
...
func IsDeadlinePeriodDone(kd *v1alpha1.KanaryStatefulset) bool {
...
func IsInitialDelayDone(kd *v1alpha1.KanaryStatefulset) (time.Duration, bool) {
	now := time.Now()
...
func getPods(kclient client.Client, reqLogger logr.Logger, KanaryStatefulsetName, KanaryStatefulsetNamespace string) ([]corev1.Pod, error) {
	pods := &corev1.PodList{}
	selector := labels.Set{
		kanaryv1alpha1.KanaryStatefulsetKanaryNameLabelKey: KanaryStatefulsetName,
	}
	listOptions := &client.ListOptions{
		LabelSelector: selector.AsSelector(),
		Namespace:     KanaryStatefulsetNamespace,
	}
